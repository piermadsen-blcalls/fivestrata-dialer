...
12. Native DID retirement caps (~1,500 dials/DID) + **individual benchmark-driven retirement** via the Telnyx number API — differentiators no current call center offers. Telnyx DIDs ~$1 (bulk 60–70¢), monthly subscription. **DID pools are per-tenant, never shared** (✅ Sean 8/19): carrier analytics score content↔number alignment, and tenants renting the platform need clean per-tenant attribution of dials/costs/reputation — shared DIDs make both murky. DIDs never move between tenants; retire and buy fresh (`docs/architecture/did-lifecycle.md`).
