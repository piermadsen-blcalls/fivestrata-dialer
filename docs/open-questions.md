...
- **DID management**: pool sizing for pilot volume, cap counters, rotation/retirement automation via Telnyx number APIs. **Open (7/22):** does the *carrier* (not Telnyx) impose a minimum hold — Ashley's "90-day" concern? Telnyx itself is a monthly subscription (swap anytime, pay the month). Design for individual benchmark-driven retirement, not block rotation. **✅ ANSWERED (7/31, td-windows replica via `scripts/did_caps_query.py`):** intraday dials per DID do NOT approach the 1,500 cap — max observed 361 (one outlier), typical top DIDs at ~25–40 dials/day, zero DID-days ≥1,000 in the visible window. Sean's instinct was right. **The bigger finding: TD rotates across a massive pool — 6,599 distinct DIDs used 7/29, 10,017 on 7/30, on ~28–55K dials/day** (sequential purchased blocks visible: 919-703-19xx, 479-504-14xx, 464-210-58/59xx, 502-630-64xx). So: no intraday retirement machinery needed — nightly Snowflake `retire_did` directive suffices; pacer least-used-DID spreading mirrors what TD already does. *Caveats:* live `vicidial_dial_log` holds only ~2 days (archive query needed for long-run confirmation); TD-windows only (KB creds would extend it); `vicidial_campaign_cid_areacodes.call_count_today` read all zeros at replication time (counter apparently unused/reset — don't rely on it). *New question this raises:* ❓ what does TD's ~10K-DID pool cost, and is huge-pool/low-use the optimal rotation curve for us vs fewer DIDs at higher daily use? (Feeds the DID cost-optimization calculus in the 7/29 meeting + `architecture/snowflake-value.md` row 1.)
