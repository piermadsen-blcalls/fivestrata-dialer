...
- **DID management**: pool sizing for pilot volume, cap counters, rotation/retirement automation via Telnyx number APIs. **Open (7/22):** does the *carrier* (not Telnyx) impose a minimum hold — Ashley's "90-day" concern? Telnyx itself is a monthly subscription (swap anytime, pay the month). Design for individual benchmark-driven retirement, not block rotation. **Open (7/29):** do intraday dials per DID actually exceed the ~1,500 cap on the human floors today? Pier thinks maybe, Sean doubts it — answerable from `vicidial_dial_log(.outbound_cid)` on the replicas (query in hand, run pending). If yes → intraday retirement machinery; if no → nightly Snowflake `retire_did` directive suffices (see `architecture/snowflake-value.md`), with intraday *spreading* handled natively by the pacer's least-used-DID pick (`architecture/concurrency-queueing.md`).
