...
| T2 | **Telnyx account + API keys** (Call Control, SIP trunking, media streaming, voice-AI product, number ordering, pricing) | ~~Gates the A/B/C architecture decision~~ *(fork decided 7/23→7/27 — no ViciDial)* — keys now gate the soundboard/TTS cost model and PRD workstream W1. **CLOSED 2026-08-03** — Sean has portal access; `TELNYX_API_KEY` + `TELNYX_PUBLIC_KEY` live in `.env`, validated via `scripts/telnyx-check.ts` (key valid; billing-read failure turned out to be **no credit card on the account** — not a user-permission issue; corporate card procurement in progress as of 2026-08-05, and it gates the test-DID purchase → roadmap M1). *Account owns **0 phone numbers** — Pier's 7/30 demo ran elsewhere; test DID purchase is the next spend, blocked on the card. Public-docs review: `architecture/telnyx-capability-review.md`. Still downstream: negotiated pricing, warm-transfer fee scope (billing surface).* | ~~Pier / IT~~ done |
