...
| D8 | **Decisions/actions needed from Sean** | (a) initial pool size + monthly DID budget cap; (b) per-tenant CNAM — display the tenant's brand on its programs' DIDs? (c) SMS-capable sub-pool now (+$0.10/mo each) or later; (d) **Number Reputation enablement** (from D1): approve the **$100/mo base fee**, pick the legal entity + authorized LOA signer, agree ToS, sign/upload the LOA (Hiya vetting ~minutes) — blocks D4 screening and D11 sweeps. Alternative if $100/mo is rejected at pilot scale: dial unscreened and let D2 decline-monitoring catch bad DIDs fast (viable on a 10–25 pool, forfeits the screening keep-rate) | Sean | **$100/mo** + fresh-query rate TBD |
