...
| 5 | **Cadence/best-time-to-call uplift** per segment (vertical × fresh-vs-revive × geo × attempt # × **day-of-week**) — ➤ parked long-term (Sean 8/17: "it makes a difference") | `queue_weights(segment, daypart weights)` | L3 rescheduler (`campaign-delivery.md` §5) places `dial_jobs.not_before`/`daypart_pref` from the weights instead of the shipped rotation default — the test-bench loop closed |
